...
<!-- <div id="menuContent" class="menuContent" style="display:none; position: absolute;background:#ffffff;border:1px solid #1ab394;">
	...
	function zTree(){
		...
		// 角色id
		$.ajax({
			url:'${pageContext.request.contextPath}/security/privilegegroup/privilegeTree',
			type:'post',
				...
				if(data!='-1'){
					...
					for(var i = 0;i < nodes.length;i++){
						for(var j = 0;j < privileges.length;j++){
							if(nodes[i].id == privileges[j].pvgId){
	...
	function submitForm(){
		...
		$.ajax({
			url: server+'/security/pvgpvggroup/save',
			type:'post',
			data:{pgId:"${pg.pgId}",nodes:checkeds.substring(1),'_csrf':$("meta[name='_csrf']").attr("content"),'_csrf_header':$("meta[name='_csrf_header']").attr("content")},
			async:false,
			success:function(data){
				if(data=='1'){
