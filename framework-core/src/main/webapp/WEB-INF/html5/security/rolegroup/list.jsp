...
<div id="condition-content" class="condition-content" style="display:none;">
	<div class="form-group"><span>角色组名称</span><input type="text" id="rgName" name="rgName"  placeholder="" class="form-control"></div>
	&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <security:authorize url="/security/rolegroup/add"><div class="form-group"><button class="btn btn-sm btn-primary" type="button" onclick="add();">添加</button></div></security:authorize>
    <security:authorize url="/security/rolegroup/delete"><div class="form-group"><button class="btn btn-sm btn-danger" type="button" onclick="dels();">删除</button></div></security:authorize>
</div>
...
<%--     <script src="<c:url value='/js/plugins/jeditable/jquery.jeditable.js'/>"></script> --%>
	...
	$(document).ready(function() {
		...
		var table = $('.data-table').DataTable( {
	        ...
	        "ajax":{
	        	"url":"${pageContext.request.contextPath}/security/rolegroup/list",
	            "type":"post",
	            "data":function(data){
	            	data.column=5;
	        ...
	        "columns": [
	            ...
	            { "title":"角色组名称","data": "rgName","sClass":"dt-body-center" },
	            { "title":"角色组描述","data": "rgDesc","sClass":"dt-body-center" },
	        ...
	        "aoColumnDefs": [ 
                ...
                { "bSortable": false, "aTargets": [ 5 ] ,"visible": <security:ifelse url="/security/rolegroup/assignation" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 6 ] ,"visible": <security:ifelse url="/security/rolegroup/detail" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 7 ] ,"visible": <security:ifelse url="/security/rolegroup/edit" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 8 ] ,"visible": <security:ifelse url="/security/rolegroup/delete" ifValue="true" elseValue="false"/>}
	        ],
	        // 默认指定列排序
	        "aaSorting": [[ 2, "asc" ]]
	    } );
	...
	function dels(){
		...
		parent.swal({title: "你确定要删除吗?",
	        ...
	        function (isConfirm) {
	            if (isConfirm) {
	            	$.ajax({
	        			url:'${pageContext.request.contextPath}/security/rolegroup/delete',
	        			type:'post',
	...
	function assignation(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '50%'],
			    content: '${pageContext.request.contextPath}/security/rolegroup/assignation?rgId='+id
			});
			return;
		}catch(e){
	...
	function add(){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['30%', '40%'],
			    content: '${pageContext.request.contextPath}/security/rolegroup/add'
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function detail(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['30%', '40%'],
			    content: '${pageContext.request.contextPath}/security/rolegroup/detail?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function edit(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['30%', '40%'],
			    content: '${pageContext.request.contextPath}/security/rolegroup/edit?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
