								...
								<div class="table-responsive table-top-border-none">
									<table class="table table-bordered">
										<tr>
											...
											<td>
												<div class="form-group form-group-table"><input type="text" placeholder="角色组名称" class="form-control" id="rgName" name="rgName"></div>
											</td>
										</tr>
										<tr>
											<td class="dt-align-right">
												<label class="control-label">角色组描述</label>
											</td>
											<td>
												<div class="form-group form-group-table"><input type="text" placeholder="角色组描述" class="form-control" id="rgDesc" name="rgDesc"></div>
											</td>
										</tr>
										<tr>
											<td class="dt-align-right">
											...
											<td>
												<div class="form-group form-group-table"><input type="text" placeholder="备注" class="form-control" id="note" name="note"></div>
											</td>
										</tr>
										<tr>
											<td colspan="2">
...
<!-- <div id="menuContent" class="menuContent" style="display:none; position: absolute;background:#ffffff;border:1px solid #1ab394;">
		                    ...
		                    stringLength: {
		   	                 ...
		   	                 callback: {
		 						...
		 						callback: function (value, validator) {
		 							...
		 							if (value!='') {
	 									$.ajax({
	 										url: server + '/security/rolegroup/isExist',
	 										type: 'post',
	 										async: false,
	 										data: {"max":"0","rgName": value,'_csrf':$("meta[name='_csrf']").attr("content"),'_csrf_header':$("meta[name='_csrf_header']").attr("content")},
	...
	function submitForm(){
		...
		if($("#form").data('bootstrapValidator').isValid()){
			$.ajax({
				url: server+'/security/rolegroup/add',
				type:'post',
				data:$("#form").serialize(),
				async:false,
				success:function(data){
