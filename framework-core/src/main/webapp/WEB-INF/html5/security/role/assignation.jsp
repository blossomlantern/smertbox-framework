...
<!-- <div id="menuContent" class="menuContent" style="display:none; position: absolute;background:#ffffff;border:1px solid #1ab394;">
	...
	function zTree(){
		...
		// 角色id
		$.ajax({
			url:'${pageContext.request.contextPath}/security/role/privilegeTree',
			type:'post',
	...
	function submitForm(){
		...
		//return;
		$.ajax({
			url: server+'/security/roleprivilege/save',
			type:'post',
			data:{roleId:"${roleId}",nodes:checkeds.substring(1),'_csrf':$("meta[name='_csrf']").attr("content"),'_csrf_header':$("meta[name='_csrf_header']").attr("content")},
			async:false,
