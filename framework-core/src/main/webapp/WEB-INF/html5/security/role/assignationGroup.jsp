				...
				<div class="ibox-content">
					<div class="form-group">
						...
						<c:forEach items="${alls}" var="element" varStatus="status">
							...
							<td><div class="checkbox-inline i-checks dialog-checkbox"><input id="${element.pgId}" name="pvg" type="checkbox" value="${element.pgId}"> <i></i> ${element.pgName} </div></td>
						</c:forEach>
						</tr>
						</table>
					</div>
...
<!-- <div id="menuContent" class="menuContent" style="display:none; position: absolute;background:#ffffff;border:1px solid #1ab394;">
	...
	$(document).ready(function() {
		 ...
		 var pvg = $("input[name='pvg']");
		 for(var i = 0;i < pvg.length;i++){
			 for(var j = 0;j < groups.length;j++){
				 if($(pvg[i]).val() == groups[j].pvggroupId){
				 	$(pvg[i]).iCheck("check");
				 }
			 }
		 }
	...
	function submitForm(){
		var checkeds = getcheckboxValueArray("pvg");
		$.ajax({
			url: server+'/security/rolepvggroup/save',
			type:'post',
			data:{roleId:"${roleId}",nodes:checkeds,'_csrf':$("meta[name='_csrf']").attr("content"),'_csrf_header':$("meta[name='_csrf_header']").attr("content")},
			async:false,
