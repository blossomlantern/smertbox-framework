...
<div class="animated">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<form id="form" method="post" class="form-horizontal">
								...
								<input type="hidden" id="roleId" name="roleId" value="${vobj.roleId}">
										...
										<tr>
											...
											<td>
												<div class="form-group form-group-table">
													<input type="text" placeholder="角色名称" class="form-control" id="roleName" name="roleName" value="${vobj.roleName}">
												</div>
											</td>
										</tr>
										<tr>
											...
											<td>
												<div class="form-group form-group-table">
													<input type="text" placeholder="角色描述" class="form-control" id="roleDesc" name="roleDesc" value="${vobj.roleDesc}"> 
												</div>
											</td>
										</tr>
										<tr>
											...
											<td>
												<div class="form-group form-group-table">
													<select class="form-control"id="roleType" name="roleType">
			                                    		<option value="dev" <c:if test="${vobj.roleType=='dev'}">selected</c:if>>开发</option>
			                                    		<option value="system" <c:if test="${vobj.roleType=='system'}">selected</c:if>>系统</option>
			                                    	</select>
												</div>
											</td>
										</tr>
		    	  		...
		    	  		validators: {
		                    ...
		                    stringLength: {
		 						...
		 						callback: function (value, validator) {
		 							...
		 							if (value!='') {
	 									$.ajax({
	 										url: server + '/security/role/isExist',
	 										type: 'post',
	 										async: false,
	 										data: {"max":"1","roleName": value,'_csrf':$("meta[name='_csrf']").attr("content"),'_csrf_header':$("meta[name='_csrf_header']").attr("content")},
	...
	function submitForm(){
		...
		$.ajax({
			...
			type: 'post',
			async: false,
			data: {"max":"1","roleName": $("#roleName").val(),'_csrf':$("meta[name='_csrf']").attr("content"),'_csrf_header':$("meta[name='_csrf_header']").attr("content")},
			success: function (data) {
				if (data == '1') {
					parent.swal("已存在", "该角色已存在!", "warning");
					return;
				}
			}
		});
