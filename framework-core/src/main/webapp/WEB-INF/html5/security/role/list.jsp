...
<div id="condition-content" class="condition-content" style="display:none;">
	<div class="form-group"><span>角色名称</span><input type="text" id="roleName" name="roleName"  placeholder="" class="form-control"></div>
	&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <security:authorize url="/security/role/add"><div class="form-group"><button class="btn btn-sm btn-primary" type="button" onclick="add();">添加</button></div></security:authorize>
    <security:authorize url="/security/role/delete"><div class="form-group"><button class="btn btn-sm btn-danger" type="button" onclick="dels();">删除</button></div></security:authorize>
</div>
...
<%--     <script src="<c:url value='/js/plugins/jeditable/jquery.jeditable.js'/>"></script> --%>
	...
	$(document).ready(function() {
		...
		var table = $('.data-table').DataTable( {
	        ...
	        "ajax":{
	        	"url":"${pageContext.request.contextPath}/security/role/list",
	            "type":"post",
	            "data":function(data){
	            	data.column=5;
	        ...
	        "columns": [
	            ...
	            { "title":"角色名称","data": "roleName","sClass":"dt-body-center" },
	            { "title":"角色描述","data": "roleDesc","sClass":"dt-body-center" },
	            { "title":"角色类型",
	              "data": "roleType",
	              "sClass":"dt-body-center",
	              "mRender":function(data,type,full) {
					  var content = (data == "dev" ? "开发" : "系统");
                      return content;
                   }  
	            },
				...
				{ "title":"权限组",
				  ...
				  "sClass":"dt-body-center",
				  "mRender":function(data,type,full) {
					  var content = "<button class=\"btn btn-sm-sm btn-primary \" type=\"button\" onclick=\"assignationGroup('"+data+"');\"><i class=\"fa fa-gears\"></i> 分配</button>";
                      return content;
                  }
				},
	            { "title":"详情",
	        ...
	        "aoColumnDefs": [ 
                ...
                { "bSortable": false, "aTargets": [ 6 ] ,"visible": <security:ifelse url="/security/role/assignation" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 7 ] ,"visible": <security:ifelse url="/security/role/assignationGroup" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 8 ] ,"visible": <security:ifelse url="/security/role/detail" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 9 ] ,"visible": <security:ifelse url="/security/role/edit" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 10 ] ,"visible": <security:ifelse url="/security/role/delete" ifValue="true" elseValue="false"/>}
	        ],
	        // 默认指定列排序
	        "aaSorting": [[ 2, "asc" ]]
	    } );
	...
	function dels(){
		...
		parent.swal({title: "你确定要删除吗?",
	        ...
	        function (isConfirm) {
	            if (isConfirm) {
	            	$.ajax({
	        			url:'${pageContext.request.contextPath}/security/role/delete',
	        			type:'post',
	...
	function assignation(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['20%', '90%'],
			    content: '${pageContext.request.contextPath}/security/role/assignation?roleId='+id
			});
			return;
		}catch(e){
	...
	function assignationGroup(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '50%'],
			    content: '${pageContext.request.contextPath}/security/role/assignationGroup?roleId='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function add(){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '45%'],
			    content: '${pageContext.request.contextPath}/security/role/add'
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function detail(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '50%'],
			    content: '${pageContext.request.contextPath}/security/role/detail?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function edit(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '45%'],
			    content: '${pageContext.request.contextPath}/security/role/edit?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
