...
<div id="condition-content" class="condition-content" style="display:none;">
	<div class="form-group"><span>职位名称</span><input type="text" id="positionName" name="positionName"  placeholder="" class="form-control"></div>
	&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <security:authorize url="/security/position/add"><div class="form-group"><button class="btn btn-sm btn-primary" type="button" onclick="add();">添加</button></div></security:authorize>
    <security:authorize url="/security/position/add"><div class="form-group"><button class="btn btn-sm btn-danger" type="button" onclick="dels();">删除</button></div></security:authorize>
</div>
...
<%--     <script src="<c:url value='/js/plugins/jeditable/jquery.jeditable.js'/>"></script> --%>
	...
	$(document).ready(function() {
		...
		var table = $('.data-table').DataTable( {
	        ...
	        "ajax":{
	        	"url":"${pageContext.request.contextPath}/security/position/list",
	            "type":"post",
	            "data":function(data){
	            	data.column=5;
	        ...
	        "columns": [
	            ...
	            { "title":"职位名称","data": "positionName","sClass":"dt-body-center" },
	            { "title":"职位描述","data": "positionDesc","sClass":"dt-body-center" },
	            { "title":"职位类型","data": "positionType","sClass":"dt-body-center",
            	  "mRender":function(data,type,full) {
					  var content = "";
					  if(data == "technology"){
						  content = "技术类";
	        ...
	        "aoColumnDefs": [ 
                ...
                { "bSortable": false, "aTargets": [ 6 ] ,"visible": <security:ifelse url="/security/position/assignationRole" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 7 ] ,"visible": <security:ifelse url="/security/position/assignationRolegroup" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 8 ] ,"visible": <security:ifelse url="/security/position/detail" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 9 ] ,"visible": <security:ifelse url="/security/position/edit" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 10 ] ,"visible": <security:ifelse url="/security/position/delete" ifValue="true" elseValue="false"/>}
	        ],
	        // 默认指定列排序
	        "aaSorting": [[ 2, "asc" ]]
	    } );
	...
	function assignationRole(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '60%'],
			    content: '${pageContext.request.contextPath}/security/position/assignationRole?id='+id
			});
			return;
		}catch(e){
	...
	function assignationRolegroup(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '60%'],
			    content: '${pageContext.request.contextPath}/security/position/assignationRolegroup?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function dels(){
		...
		parent.swal({title: "你确定要删除吗?",
	        ...
	        function (isConfirm) {
	            if (isConfirm) {
	            	$.ajax({
	        			url:'${pageContext.request.contextPath}/security/position/delete',
	        			type:'post',
	...
	function add(){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '60%'],
			    content: '${pageContext.request.contextPath}/security/position/add'
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function detail(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '60%'],
			    content: '${pageContext.request.contextPath}/security/position/detail?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function edit(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['40%', '60%'],
			    content: '${pageContext.request.contextPath}/security/position/edit?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
