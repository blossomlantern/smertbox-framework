...
<div class="animated">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<form id="form" method="post" class="form-horizontal">
								...
								<input type="hidden" id="pvgId" name="pvgId" value="${vobj.pvgId}">
								<div class="table-responsive table-top-border-none">
									<table class="table table-bordered">
										<tr>
											...
											<td>
												<div class="form-group form-group-table">
													<input type="text" placeholder="权限名称" class="form-control" id="pvgName" name="pvgName" value="${vobj.pvgName}">
												</div>
											</td>
										</tr>
										<tr>
											...
											<td>
												<div class="form-group form-group-table">
													<input type="text" placeholder="父级权限" class="form-control" id="privilegeTree" onclick="showMenu(this);" value="${vobj.pvgPidName}" readonly>
                                     				<input type="hidden" id="pvgPid" name="pvgPid" value="${vobj.pvgPid}">
												</div>
											</td>
										</tr>
										<tr>
											...
											<td>
												<div class="form-group form-group-table">
													<input type="text" placeholder="对应资源" class="form-control" id="resourceTree" onclick="showMenu(this);" value="${vobj.resName}" readonly>
										...
										<tr>
											...
											<td>
												<div class="form-group form-group-table">
													<input type="text" placeholder="权限描述" class="form-control" id="pvgDesc" name="pvgDesc" value="${vobj.pvgDesc}">
												</div>
											</td>
										</tr>
										<tr>
											...
											<td>
												<div class="form-group form-group-table">
													<select class="form-control"id="pvgType" name="pvgType">
			                                    		<option value="url" <c:if test="${vobj.pvgType=='url'}">selected</c:if>>URL权限</option>
			                                    		<option value="button" <c:if test="${vobj.pvgType=='button'}">selected</c:if>>按钮权限</option>
			                                    	</select>
												</div>
											</td>
										</tr>
	...
	$(document).ready(function() {
		...
		// 添加权限树下拉单选控件
		zTreeSingleCheck("pvgPid","privilegeTree","/security/privilege/privilegeTree","${vobj.pvgPid}");
		// 添加资源树下拉单选控件
		zTreeSingleCheck("resId","resourceTree","/security/resource/resourceTree","${resId}");
	});
	...
	function submitForm(){
		...
		$.ajax({
			...
			type: 'post',
			async: false,
			data: {"max":"1","resId": $("#resId").val(),'_csrf':$("meta[name='_csrf']").attr("content"),'_csrf_header':$("meta[name='_csrf_header']").attr("content")},
			success: function (data) {
				if (data == '1') {
					parent.swal("已存在", "该资源已经绑定过权限!", "warning");
					return;
				}
			}
		});
		...
		if($("#form").data('bootstrapValidator').isValid()){
			$.ajax({
				url: server+'/security/privilege/edit',
				type:'post',
				data:$("#form").serialize(),
				async:false,
				success:function(data){
