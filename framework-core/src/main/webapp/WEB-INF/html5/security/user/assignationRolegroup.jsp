				...
				<div class="ibox-content">
					<div class="form-group">
						...
						<c:forEach items="${alls}" var="element" varStatus="status">
							...
							<td><div class="checkbox-inline i-checks dialog-checkbox"><input id="${element.rgId}" name="rolegroup" type="checkbox" value="${element.rgId}"> <i></i> ${element.rgName} </div></td>
						</c:forEach>
						</tr>
						</table>
					</div>
...
<!-- <div id="menuContent" class="menuContent" style="display:none; position: absolute;background:#ffffff;border:1px solid #1ab394;">
	...
	$(document).ready(function() {
		 ...
		 var rolegroup = $("input[name='rolegroup']");
		 for(var i = 0;i < rolegroup.length;i++){
			 for(var j = 0;j < rolegroups.length;j++){
				 if($(rolegroup[i]).val() == rolegroups[j].rgId){
				 	$(rolegroup[i]).iCheck("check");
				 }
			 }
		 }
	});
	...
	function submitForm(){
		var checkeds = getcheckboxValueArray("rolegroup");
		$.ajax({
			url: server+'/security/userrolegroup/save',
			type:'post',
