...
<div id="condition-content" class="condition-content" style="display:none;">
	<div class="form-group"><span>菜单名称</span><input type="text" id="menuName" name="menuName"  placeholder="" class="form-control"></div>
	&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <security:authorize url="/security/menu/delete"><div class="form-group"><button class="btn btn-sm btn-danger" type="button" onclick="dels();">删除</button></div></security:authorize>
</div>
...
<%--     <script src="<c:url value='/js/plugins/jeditable/jquery.jeditable.js'/>"></script> --%>
	...
	$(document).ready(function() {
		...
		var table = $('.data-table').DataTable( {
	        ...
	        "ajax":{
	        	"url":"${pageContext.request.contextPath}/security/menu/list",
	            "type":"post",
	            "data":function(data){
	            	data.column=5;
	        ...
	        "columns": [
	            ...
	            { "title":"菜单名称","data": "menuName","sClass":"dt-body-center" },
	            { "title":"菜单描述","data": "menuDesc","sClass":"dt-body-center" },
	            { "title":"菜单访问地址","data": "menuUrlValue" },
	            { "title":"菜单类型","data": "menuType","sClass":"dt-body-center" },
	        ...
	        "aoColumnDefs": [ 
                ...
                { "bSortable": false, "aTargets": [ 7 ] ,"visible": <security:ifelse url="/security/menu/detail" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 8 ] ,"visible": <security:ifelse url="/security/menu/edit" ifValue="true" elseValue="false"/>},
                { "bSortable": false, "aTargets": [ 9 ] ,"visible": <security:ifelse url="/security/menu/delete" ifValue="true" elseValue="false"/>}
	        ],
	        // 默认指定列排序
	        "aaSorting": [[ 2, "asc" ]]
	    } );
	...
	function dels(){
		...
		parent.swal({title: "你确定要删除吗?",
	        ...
	        function (isConfirm) {
	            if (isConfirm) {
	            	$.ajax({
	        			url:'${pageContext.request.contextPath}/security/menu/delete',
	        			type:'post',
	...
	function detail(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['50%', '80%'],
			    content: '${pageContext.request.contextPath}/security/menu/detail?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function edit(id){
		try{
			self.parent.layer.open({
			    ...
			    shade: 0.5,
			    area: ['50%', '80%'],
			    content: '${pageContext.request.contextPath}/security/menu/edit?id='+id
			});
			return;
		}catch(e){
			console.log("子页面单独运行."+e);
	...
	function zTree(){
		...
		var zNodes = null;
		$.ajax({
			url:'${pageContext.request.contextPath}/security/menu/menuTree',
			type:'post',
