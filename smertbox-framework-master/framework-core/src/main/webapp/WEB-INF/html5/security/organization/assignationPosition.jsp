				...
				<div class="ibox-content">
					<div class="form-group">
						...
						<c:forEach items="${map}" var="entry">
							...
							<tr>
								<c:forEach items="${entry.value}" var="element" varStatus="status">
									<c:if test="${status.index != 0 && status.index%3 == 0}"></tr><tr></c:if>
									<td><div class="checkbox-inline i-checks dialog-checkbox"><input id="${element.positionId}" name="position" type="checkbox" value="${element.positionId}"> <i></i> ${element.positionName} </div></td>
								</c:forEach>
							</tr>
						</c:forEach>
						</table>
...
<!-- <div id="menuContent" class="menuContent" style="display:none; position: absolute;background:#ffffff;border:1px solid #1ab394;">
	...
	$(document).ready(function() {
		 ...
		 for(var i = 0;i < position.length;i++){
			 for(var j = 0;j < positions.length;j++){
				 if($(position[i]).val() == positions[j].positionId){
				 	$(position[i]).iCheck("check");
				 }
			 }
		 }
	});
	...
	function submitForm(){
		var checkeds = getcheckboxValueArray("position");
		$.ajax({
			url: server+'/security/orgposition/save',
			type:'post',
			data:{orgId:"${orgId}",nodes:checkeds,'_csrf':$("meta[name='_csrf']").attr("content"),'_csrf_header':$("meta[name='_csrf_header']").attr("content")},
			async:false,
